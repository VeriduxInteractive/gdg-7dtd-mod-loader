...
const gradleUserHome = process.env.GDG_GRADLE_USER_HOME
  || path.join(process.env.LOCALAPPDATA || repoRoot, "GoldenDaysGaming", "BuildTools", "gradle-minecraft-1.20.1");

const javaCandidates = [
  process.env.GDG_JAVA17_HOME ? path.join(process.env.GDG_JAVA17_HOME, "bin", "java.exe") : "",
  ...
  process.env.LOCALAPPDATA ? path.join(process.env.LOCALAPPDATA, "GoldenDaysGaming", "BuildTools", "temurin-17", "bin", "java.exe") : "",
  "java"
].filter(Boolean);

function isJava17Jdk(candidate) {
  if (candidate !== "java" && !fs.existsSync(candidate)) {
    return false;
  }
  const version = spawnSync(candidate, ["-version"], { encoding: "utf8", windowsHide: true });
  const versionText = `${version.stdout || ""}\n${version.stderr || ""}`;
  if (version.status !== 0 || !/version\s+"17(?:\.|\")/.test(versionText)) {
    return false;
  }
  ...
  return compiler.status === 0 && /javac\s+17(?:\.|\s|$)/.test(`${compiler.stdout || ""}\n${compiler.stderr || ""}`);
}

const javaExecutable = javaCandidates.find(isJava17Jdk);
if (!javaExecutable) {
  throw new Error("A Java 17 JDK is required to build the Forge 1.20.1 Quick Join addon. Set GDG_JAVA17_HOME to a Java 17 JDK.");
}
