...
Minecraft packs managed by CurseForge may set `modManager: "curseforge"` and `packUrl` to the official pack page. Existing Prism instances remain supported with `modManager: "prism"`. Their `syncUrl` can use a packaged manifest such as `bundled://minecraft-otherworld-v8-hf2.json`; this validates pack identity without redistributing third-party mod files.

The bundled `minecraft-bootstrap.json` identifies the official CurseForge standalone installer, its required Authenticode publisher, the exact project/file IDs, and the first-party `curseforge://install` URI. When no compatible instance is detected, Make Me Ready downloads the installer over HTTPS, verifies its valid `Overwolf Ltd` signature, installs CurseForge, requests the exact pack, waits for the matching completed profile, verifies the pack files, and then adds only explicitly managed addons. For Otherworld v8 HF2, those are the client-only Forge 1.20.1 `GDG-Quick-Join.jar` and the client/server `SecurityCraft-1.10.2.1.jar`. SecurityCraft file `8336490` is MIT licensed and its license notice is packaged beside the JAR. Do not add restricted upstream pack JARs to the loader package.

The Minecraft migration prompt is limited to pack project IDs explicitly listed as prior GDG profiles. Players can keep a prior profile or separately confirm moving it to the Windows Recycle Bin before the new profile is created. Revalidate the pack identity and directory structure immediately before removal; never infer deletion candidates from folder names or include unrelated modpacks.
