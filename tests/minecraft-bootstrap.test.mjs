...
describe("GDG-managed Minecraft bootstrap", () => {
  it("pins the signed CurseForge installer and exact Otherworld pack identity", async () => {
    ...
    expect(manifest.pack.projectId).toBe("1418133");
    expect(manifest.pack.fileId).toBe("8074976");
    expect(manifest.pack.serverPackFileId).toBe("8074983");
    expect(manifest.pack.installUri).toBe("curseforge://install?addonId=1418133&fileId=8074976");
    expect(manifest.pack.minimumInstalledAddons).toBeGreaterThanOrEqual(390);
    expect(manifest.pack.minimumModFiles).toBeGreaterThanOrEqual(390);
    expect(manifest.pack.requiredPaths).toContain("configureddefaults");
    expect(manifest.minimumFreeBytes).toBeGreaterThanOrEqual(8 * 1024 ** 3);
  });

  it("installs through CurseForge, verifies the instance, and preserves migration choices", async () => {
    ...
    expect(main).toContain('projectId: "310396"');
    expect(main).toContain("Keep Old & Create Otherworld");
    expect(main).toContain("Move Old to Recycle Bin & Create Otherworld");
